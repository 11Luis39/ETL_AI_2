...
CIUDADES_CONFIG = {
    ...
    "Tiquipaya": {
        "tier": 2,
        "n_clusters": 1,
        "bbox": {"lat": (-17.4, -17.2), "lng": (-66.3, -66.1)},
    },
    ...
    "La Guardia": {
        "tier": 2,
        "n_clusters": 1,
        "bbox": {"lat": (-17.9, -17.7), "lng": (-63.4, -63.2)},
    },
    "Warnes": {"tier": 2, "n_clusters": 1, "bbox": {"lat": (-17.6, -17.4), "lng": (-63.3, -63.1)}},
    "Quillacollo": {
        "tier": 2,
        "n_clusters": 1,
        "bbox": {"lat": (-17.5, -17.3), "lng": (-66.3, -66.1)},
    },
    "Samaipata": {
        "tier": 2,
        "n_clusters": 1,
        "bbox": {"lat": (-18.3, -18.1), "lng": (-63.9, -63.7)},
    },
    ...
    "Satelite Norte": {
        "tier": 2,
        "n_clusters": 1,
        "bbox": {"lat": (-17.6, -17.4), "lng": (-63.2, -63.0)},
    },
}
...
def guardar_centroides(modelo: KMeans, ciudad: str, n_clusters: int, pg_engine):
    ...
    with pg_engine.begin() as conn:
        conn.execute(text("DELETE FROM zona_clusters WHERE ciudad = :ciudad"), {"ciudad": ciudad})
...
def procesar_ciudad(ciudad: str, config: dict, crm_engine, pg_engine):
    ...
    if tier == 1:
        ...
        n_clusters = 1
        log.info("  → Tier 2: usando 1 cluster (centroide)")
...
if __name__ == "__main__":
    ...
    log.info("  Usando ventas + alquileres de los últimos tres años calendario")
    log.info("=" * 55)
    ...
    log.info("  Ya podés correr: python -m etl.main")
