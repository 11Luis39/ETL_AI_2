...
edges["nombres_lista"] = edges["name"].apply(
    lambda val: [str(v).strip().lower() for v in val]
    if isinstance(val, list)
    else (
        [p.strip().lower() for p in val.replace(";", ",").split(",")]
        if isinstance(val, str)
        else []
    )
)

mask = edges["nombres_lista"].apply(lambda nombres: "avenida blooming" in nombres)
